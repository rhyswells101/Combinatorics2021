...
pickle_in = open("../P_n_Epsilon/P_5_extentions", "rb")
P_5_extentions = pickle.load(pickle_in)
P_5=P_5_extentions

G = SymmetricGroup(5)
# print(G.order())
perm_5 = list(G.generate_schreier_sims(af=True))
...
for fun_var in P_5:
    ...
    col1=q_1_sets[slice(120)]
    col2=q_1_sets[slice(120,240)]
    col3=q_1_sets[slice(240,360)]
    col4=q_1_sets[slice(360,480)]
    col5=q_1_sets[slice(480,600)]

    col6=q_2_sets[slice(120)]
    col7=q_2_sets[slice(120,240)]
    col8=q_2_sets[slice(240,360)]
    col9=q_2_sets[slice(360,480)]
    col10=q_2_sets[slice(480,600)]
    col11=q_2_sets[slice(600,720)]
    col12=q_2_sets[slice(720,840)]
    col13=q_2_sets[slice(840,960)]
    col14=q_2_sets[slice(960,1080)]
    col15=q_2_sets[slice(1080,1200)]


    col16=q_3_sets[slice(120)]
    col17=q_3_sets[slice(120,240)]
    col18=q_3_sets[slice(240,360)]
    col19=q_3_sets[slice(360,480)]
    col20=q_3_sets[slice(480,600)]
    col21=q_3_sets[slice(600,720)]
    col22=q_3_sets[slice(720,840)]
    col23=q_3_sets[slice(840,960)]
    col24=q_3_sets[slice(960,1080)]
    col25=q_3_sets[slice(1080,1200)]

    col26=q_4_sets[slice(120)]
    col27=q_4_sets[slice(120,240)]
    col28=q_4_sets[slice(240,360)]
    col29=q_4_sets[slice(360,480)]
    col30=q_4_sets[slice(480,600)]


    col31=q_5_sets[slice(120)]

    set_orbit=set()

    for i in range(G.order()): #check range
        l_keypairs_i=[col1[i],col2[i],col3[i],col4[i],col5[i],col6[i],col7[i],col8[i],col9[i],col10[i],col11[i],col12[i],col13[i],col14[i],col15[i],col16[i],col17[i],col18[i],col19[i],col20[i],col21[i],col22[i],col23[i],col24[i],col25[i],col26[i],col27[i],col28[i],col29[i],col30[i],col31[i]]
        dict1={}
...
data_ext_minus.to_excel(r'C:\Users\RhysL\PycharmProjects\Combinatorics2021\Group_Action_P_5_Database.xlsx', index = False)
